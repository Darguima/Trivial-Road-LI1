module Gloss_Functions.DesenhaEstados.DesenhaJogoFluido where
...
desenhaEstadoJogo (Jogo (Jogador (posX, posY)) mapa, texturas, tamanhoJanela, _, pontuacaoAtual, pontuacoes, larguraMapa, frameAtual,y) = do
  ...
  where desenharMapa :: Mapa -> Texturas -> Float -> Float -> [Picture]
        ...
        desenharMapa (Mapa l [linha]) texturas xInicial yInicial = desenhaLinhaFluida linhaNormal terreno frameAtual xInicial
          where linhaNormal = desenharLinha linha xInicial yInicial texturas
                terreno = fst linha

        desenharMapa (Mapa l (linhas:outrasLinhas)) texturas xInicial yInicial = 
          desenhaLinhaFluida linhaNormal terreno frameAtual xInicial ++ desenharMapa (Mapa l (init (linhas:outrasLinhas))) texturas xInicial (yInicial + tamanhoChunk)
          where linhaNormal = desenharLinha (last outrasLinhas) xInicial yInicial texturas
